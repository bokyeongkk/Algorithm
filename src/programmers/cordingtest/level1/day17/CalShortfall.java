...
public class CalShortfall {
    ...
    public static void main(String[] args) {
        ...
        CalShortfall calShortfall = new CalShortfall();
        long result = calShortfall.solution(price, money, count);
    ...
    public long solution(int price, int money, int count) {
        ...
        if(money < total) {        // 가진 돈보다 비용이 많이 들 경우 (현재 자신이 가지고 있는 금액에서 얼마가 모자라는지 리턴)
            answer = total-money;
        } else {                   // 금액이 부족하지 않으면 0을 리턴
            answer = 0;
        }

...
public class Costume {
    ...
    public static void main(String[] args) {
        ...
        int result = costume.solution(clothes);
        System.out.println("서로 다른 옷의 조합의 수 "+result);
    }
    ...
    public int solution(String[][] clothes) {
        ...
        for (String[] clothe : clothes) {
            ...
            /*
                ...
                : 'A'라는 Key에 해당하는 Value가 있으면 가져오고, 아닐 경우 0을 Default로 지정하여 사용하겠다는 의미의 함수
             */
            map.put(type, map.getOrDefault(type, 0) + 1);
            // Value는 곧 옷 종류의 가짓수가 되기 때문에, 이전에 값이 있었으면 기존 값에 + 1을 하고 없었으면 1을 입력하게 된다.
        }
        ...
        int answer = 1;
        Iterator<Integer> item = map.values().iterator();
        /*
            Map - Iterator(반복자)
            자바의 컬렉션(Collection)에 저장되어 있는 요소들을 순회하는 인터페이스
            -> 컬렉션 프레임워크에 대해 공통으로 사용이 가능하고 사용법이 간단하다.

            .hasNext() : 다음 요소가 있는지 판단한다.
            .next() : 다음 요소를 가져온다.
         */

        while(item.hasNext()) {
            // +1을 해줌으로서 한 종류를 입지 않은 경우도 고려한다.
            answer *= item.next().intValue() + 1;
        }

        // 아무 종류의 옷도 입지 않는 경우를 제외해준다.
        // (ex. headgear 와 eyewear 둘 다 입지 않은 경우)
        return answer - 1;
    }
}
